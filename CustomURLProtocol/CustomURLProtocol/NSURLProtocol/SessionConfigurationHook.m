//
//  SessionConfigurationHook.m
//  WeakDemo
//
//  Created by WGP on 2020/6/6.
//  Copyright © 2020 WGP. All rights reserved.
//

#import "SessionConfigurationHook.h"
#import <objc/runtime.h>
#import "CustomURLProtocol.h"

@implementation SessionConfigurationHook

+ (SessionConfigurationHook *)defaultConfiguration {
    static SessionConfigurationHook *staticConfiguration;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        staticConfiguration=[[SessionConfigurationHook alloc] init];
    });
    return staticConfiguration;
}

- (instancetype)init {
    self = [super init];
    if (self) {
        self.isExchanged = NO;
    }
    return self;
}

- (void)performExchange {
    if (!self.isExchanged) {
        self.isExchanged=YES;
        Class cls = NSClassFromString(@"__NSCFURLSessionConfiguration") ?: NSClassFromString(@"NSURLSessionConfiguration");
        [self swizzleSelector:@selector(protocolClasses) fromClass:cls toClass:[self class]];
    }
}

- (void)unExchange {
    if (self.isExchanged) {
        self.isExchanged=NO;
        Class cls = NSClassFromString(@"__NSCFURLSessionConfiguration") ?: NSClassFromString(@"NSURLSessionConfiguration");
        [self swizzleSelector:@selector(protocolClasses) fromClass:cls toClass:[self class]];
    }
}

- (void)swizzleSelector:(SEL)selector fromClass:(Class)original toClass:(Class)stub {
    Method originalMethod = class_getInstanceMethod(original, selector);
    Method stubMethod = class_getInstanceMethod(stub, selector);
    if (!originalMethod || !stubMethod) {
        [NSException raise:NSInternalInconsistencyException format:@"Couldn't load NEURLSessionConfiguration."];
    }
    method_exchangeImplementations(originalMethod, stubMethod);
}

- (NSArray *)protocolClasses {
    // 如果还有其他的监控protocol，也可以在这里加进去
    return @[[CustomURLProtocol class]];
}

@end
